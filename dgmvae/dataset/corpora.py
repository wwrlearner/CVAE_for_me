...
class PTBCorpus(object): # 预处理，将文本转换成机器可以学习的形式
    logger = logging.getLogger()

    def __init__(self, config):
        self.config = config
        self._path = config.data_dir # 指定数据文件所在目录
        self.max_utt_len = config.max_utt_len # 设置对话的最大长度
        self.tokenize = get_chat_tokenize() # 获取一个分词器
        self.train_corpus = self._read_file(os.path.join(self._path, 'ptb.train.txt')) # 读取 PTB 数据集的训练集、验证集和测试集
        ...
        self._build_vocab(config.max_vocab_cnt) # 根据 max_vocab_cnt 构建词汇表。
        self.unk = "<unk>" # 定义一个未知词的标记。
        print("Done loading corpus")

    def _read_file(self, path): # 读取文件
        with open(path, 'r') as f:
            lines = f.readlines()

        return self._process_data(lines)

    def _process_data(self, data): # 预处理数据，储存句子内容和标识
        all_text = []
        all_lens = []
        for line in data:
    ...
    def _to_id_corpus(self, data): # 将整个数据集中的将每个句子中的单词用词汇表中的 ID 代替，形成适合模型输入的格式。
        results = []
        for line in data:
...
class NeuroCorpus(object):
    ...
    def __init__(self, config):
        ...
        # 从文件中读取数据
        data_file = os.path.join(self._path, 'sim_100d_poisson_disc_label.npz')
        dat = np.load(data_file)  # 读取 npz 文件
        ...
        # 将数据切分成多个片段，形状为 50*200
        self.x_all = self.x_true.reshape(50, 200, -1)
        self.u_all = self.u_true.reshape(50, 200, -1)

        # 训练集、验证集、测试集划分
        self.x_train = self.x_all[:40]
        self.u_train = self.u_all[:40]
        self.x_valid = self.x_all[40:45]
        self.u_valid = self.u_all[40:45]
        self.x_test = self.x_all[45:]
        self.u_test = self.u_all[45:]

        print("Data loaded: Train size {}, Validation size {}, Test size {}".format(
            self.x_train.shape, self.x_valid.shape, self.x_test.shape
        ))

    def _to_id_corpus(self, x_data, u_data):
        # 将数据转换为合适的结构，以便后续加载
        results = []
        for i in range(x_data.shape[0]):
            # Pack 数据，每个条目包含神经元信号和对应的标签
            results.append(Pack(x=x_data[i], u=u_data[i]))
        return results

    def get_corpus(self):
        # 将训练集、验证集、测试集转换为统一的 Pack 结构
        train_data = self._to_id_corpus(self.x_train, self.u_train)
        valid_data = self._to_id_corpus(self.x_valid, self.u_valid)
        test_data = self._to_id_corpus(self.x_test, self.u_test)
        return Pack(train=train_data, valid=valid_data, test=test_data)
