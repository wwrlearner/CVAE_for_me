...
def train(model, train_feed, valid_feed, test_feed, config, evaluator, gen=None):
    ...
    while True:
        train_feed.epoch_init(config, verbose=done_epoch == 0, shuffle=True) # train_feed.epoch_init 用于初始化训练数据加载器，特别是在每个 epoch 的开始。它确保数据在新一轮训练时被正确加载，通常包括数据随机打乱、数据指针重置等操作
        while True:
            sub_iter = 1
            burn_num_words = 0
            burn_pre_loss = 1e4
            burn_cur_loss = 0
            break_while = False
            if aggressive_flag: # 激进训练的目标是在不同阶段重点训练模型中的不同部分，以此来减少训练不稳定或某些模块的训练滞后的问题。
                # Training posterior
            ...
            if model.flush_valid: # 用于控制是否需要清除之前的验证损失（validation loss）记录，并重置验证状态。这个变量的作用在于帮助模型避免陷入局部最优解或训练过程中的一些不理想状态。
